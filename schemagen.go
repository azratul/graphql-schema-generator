...
func init(){
    dsn = flag.String("dsn", "", "Data source name, ex: schemagen -dsn=\"(DESCRIPTION=(LOAD_BALANCE=ON)(FAILOVER=ON)(ADDRESS=(PROTOCOL={PROTOCOL})(HOST={HOST})(PORT={PORT}))(CONNECT_DATA=(SERVER=DEDICATED)(SERVICE_NAME={SERVICE_NAME})))\"")
    entities = flag.String("entities", "", "Array of entities(comma separated), ex: 'schemagen -entities=table1,table2,table3'")
    user = flag.String("user", "", "Database user, ex: schemagen -user=\"DB_USER\"")
    password = flag.String("password", "", "Database password, ex: schemagen -password=\"PASSWORD1234\"")
